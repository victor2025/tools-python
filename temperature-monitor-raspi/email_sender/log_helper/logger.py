...
# logger cache
_loggerMap = {}
def getLogger(filename = 'python/email-sender.log',level='info')->logging.log:
    logger = None
    try:
        logger = _loggerMap[filename]
    except:
        logger = _get_logger(filename, level)
        _loggerMap[filename] = logger  
    return logger